...
def _add_action_subparsers(action_sub: argparse._SubParsersAction) -> None:
    ...
    r = action_sub.add_parser(
        "ratchet",
        help="Distill a session into raw/notes/session/<sid>.md (LLM call).",
    )
    r.add_argument("--cwd", default=os.getcwd())
    r.add_argument("--session", required=True, help="Session id (or prefix).")
    r.add_argument("--dry-run", action="store_true")
    r.add_argument("--model", default=None)
    r.set_defaults(_handler=_handle_ratchet)
...
async def _handle_status(args: argparse.Namespace) -> int:
    ...
    print()
    print(f"notes/        {n_hand_notes:>5} hand-written, {n_session} from /ratchet")
    ...
    if n_hand_notes == 0 and n_session == 0 and any(counts.values() if counts else [0]):
        recos.append(
            "no hand-written notes and no /ratchet output — start a chat with "
            "`agent-forge --ratchet` so insights compound across sessions."
        )
...
def _make_provider_and_model(args: argparse.Namespace):
    """Resolve the provider + model for compile/ratchet/compact subcommands.
...
# ── Compile / ratchet / compact / maintain handlers ──────────────────────────
...
async def _handle_ratchet(args: argparse.Namespace) -> int:
    from ...session import resolve_session_spec
    from ..ratchet import ratchet_session

    root = Path(args.cwd).resolve()
    sid = resolve_session_spec(args.session, str(root))
    if sid is None:
        # Allow passing a literal session id even if it isn't in the cwd index.
        sid = args.session
    provider, model = _make_provider_and_model(args)
    if provider is None:
        return 1

    print(f"ratchet → session {sid[:12]}…")
    res = await ratchet_session(root, sid, provider, model, dry_run=args.dry_run)
    if res.error:
        print(f"error: {res.error}", file=sys.stderr)
        return 1
    if not res.wrote:
        print(res.insights_text or "nothing to ratchet")
        return 0
    print(f"wrote {res.output_path.relative_to(root) if res.output_path else '?'}")
    return 0
